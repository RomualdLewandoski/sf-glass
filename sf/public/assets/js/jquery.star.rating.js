(function ($) {
    $.fn.showStatic = function(value, options){
        var obj = this;
        var settings = $.extend({
            max: 5,
            half: true,
        ...
        for (var i = 1; i <= settings.max; i++) {
            var star = $('<i/>').addClass('material-icons').html(this.settings.icon + '_border').data('rating', i).appendTo(this).hover(
                function (e) {
